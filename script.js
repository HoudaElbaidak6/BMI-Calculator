  ...
  if (weightValue == 0 || heightValue == 0) {
    ...
    if (isNaN(weightValue) || isNaN(heightValue)) {
      ...
      if (BMI < 18.5) {
        ...
        output.textContent = `Your BMI is ${BMI} which means your weight is normal. `;
      } else if (BMI >= 25 && BMI < 30) {
        output.textContent = `Your BMI is ${BMI} which means you're overweight, stop eating !!.  `;
      } else if (BMI >= 30 && BMI < 40) {
        output.textContent = `Your BMI is ${BMI} which means you're Obese , damn you really let yourself go , start a diet ! `;
      } else {
